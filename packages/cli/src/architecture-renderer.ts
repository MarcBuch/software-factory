...
function changePlanSteps(input: PlanInput): string[] {
  if (input.changePlanSteps?.some((step) => step.trim()))
    return input.changePlanSteps.filter((step) => step.trim());
  const plan = input.changePlan.trim();
  const markers = [...plan.matchAll(/(\d+)[.)]\s+/g)];
  if (markers.length < 2 || markers[0].index !== 0) return [plan];
  for (let index = 0; index < markers.length; index++) {
    if (Number(markers[index][1]) !== index + 1) return [plan];
    if (index === 0) continue;
    const between = plan.slice(
      (markers[index - 1].index ?? 0) + markers[index - 1][0].length,
      markers[index].index,
    );
    const afterNewline = between.slice(between.lastIndexOf("\n") + 1).trim();
    if (between.includes("\n") ? afterNewline !== "" : !/[.!?]$/.test(afterNewline)) return [plan];
  }
  const steps = markers.map((marker, index) => {
    const start = (marker.index ?? 0) + marker[0].length;
    const end = markers[index + 1]?.index ?? plan.length;
    return plan.slice(start, end).trim();
  });
  return steps.every(Boolean) ? steps : [plan];
}

function renderChangePlan(input: PlanInput): string {
  const steps = changePlanSteps(input)
    .map((step) => `<li>${text(step)}</li>`)
    .join("");
  return `<p class="decision-label">Decision</p><strong>${text(input.intent)}</strong><p class="outline-label">Change outline</p><ol class="change-outline">${steps}</ol>`;
}
