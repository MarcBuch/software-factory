...
const approval = <T extends z.ZodObject<any>>(schema: T) => schema.superRefine((value: any, ctx) => {
  ...
  if ((value.status === "approved" || value.status === "superseded") && !hasApproval)
    ctx.addIssue({ code: "custom", path: ["approvedAt"], message: "approvedAt is required for approved or superseded records" });
  if ((value.status === "draft" || value.status === "archived") && hasApproval)
    ctx.addIssue({ code: "custom", path: ["approvedAt"], message: "approvedAt is only valid for approved or superseded records" });
  if (value.updatedAt < value.createdAt)
    ctx.addIssue({ code: "custom", path: ["updatedAt"], message: "updatedAt must be >= createdAt" });
});

export const PlanRevisionSchema = approval(z.object({
  id: planId,
  missionTitle: text,
  verificationMode: z.enum(["fast", "standard", "exhaustive"]),
  milestones: z.array(z.object({ key: text, title: text }).strict()),
...
export function resolveDependencies(keys: string[], taskIds: ReadonlyMap<string, string>): string[] {
  return keys.map((key) => { const id = taskIds.get(key); if (!id) throw Error(`Dependency task not found: ${key}`); return id; });
}
 ...
 */
export function validatePlansAgainstMissions(plans: Plan[], missions: MissionReference[]): Plan[] {
  const groups = new Map<string, Plan[]>();
  for (const plan of plans) {
    PlanSchema.parse(plan);
    const milestoneKeys = new Set(plan.milestones.map(m => m.key));
    if (milestoneKeys.size !== plan.milestones.length) throw Error(`Duplicate milestone key in plan ${plan.id}`);
    ...
    for (const step of plan.steps) { if (!milestoneKeys.has(step.milestoneKey)) throw Error(`Unknown milestone key: ${step.milestoneKey}`); if (new Set(step.dependsOn).size !== step.dependsOn.length) throw Error(`Duplicate dependency in plan ${plan.id}`); for (const dep of step.dependsOn) if (!taskKeys.has(dep)) throw Error(`Dependency is not represented in plan ${plan.id}: ${dep}`); }
    const graph = new Map(plan.steps.map(s => [s.key, s.dependsOn])); const visiting = new Set<string>(), visited = new Set<string>();
    const visit = (key: string) => { if (visiting.has(key)) throw Error(`Cyclic plan dependency involving task: ${key}`); if (visited.has(key)) return; visiting.add(key); for (const dep of graph.get(key) ?? []) visit(dep); visiting.delete(key); visited.add(key); }; for (const key of graph.keys()) visit(key);
    (groups.get(plan.id) ?? groups.set(plan.id, []).get(plan.id)!).push(plan);
  }
  for (const [id, group] of groups) { const ordered = [...group].sort((a,b)=>a.revision-b.revision); ordered.forEach((p,i)=>{if(p.revision!==i+1)throw Error(`Plan revisions must be contiguous for ${id}`);}); for(let i=1;i<ordered.length;i++) if(ordered[i].createdAt<ordered[i-1].createdAt||ordered[i].updatedAt<ordered[i-1].updatedAt) throw Error(`Revision timestamps must be chronological for ${id}`); if(ordered.filter(p=>p.status==="approved").length>1)throw Error(`Only one revision may be approved for ${id}`); if(ordered.slice(0,-1).some(p=>p.status==="draft"))throw Error(`Prior revisions must be superseded or archived for ${id}`); if(ordered.slice(0,-1).some(p=>p.status==="approved"))throw Error(`Only latest revision may be approved for ${id}`); }
  return plans;
}

export const PlansMetadataSchema = z.object({ type: z.literal("plans-metadata"), schemaVersion: z.literal(1) }).strict();
...
export async function plansPath() { return join(await projectRoot(), ".factory", "plans.jsonl"); }

export async function loadPlans(file: string | undefined, missions: MissionReference[]): Promise<Plan[]> {
...
export async function savePlansUnlocked(plans: Plan[], file: string) {
  ...
  const data = [JSON.stringify({ type: "plans-metadata", schemaVersion: 1 }), ...plans.map((plan) => JSON.stringify(PlanSchema.parse(plan)))].join("\n") + "\n";
  const temporary = `${file}.tmp.${process.pid}.${crypto.randomUUID()}`;
  try { await writeFile(temporary, data, { mode: 0o600 }); await rename(temporary, file); }
  finally { await rm(temporary, { force: true }); }
}

export async function savePlans(plans: Plan[], missions: MissionReference[], file?: string) {
  file ??= await plansPath();
  return withFactoryLock(() => { validatePlansAgainstMissions(plans, missions); return savePlansUnlocked(plans, file!); });
}
...
export async function ensurePlansMetadataUnlocked(file: string) { if (!existsSync(file)) await savePlansUnlocked([], file); }
