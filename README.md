...
If the request argument is omitted, the request is read from stdin. Built-in roster entries include the read-only `scout` and `planner` agents. The planner is available only when run from this repository because it uses the local `.opencode/agents/plan-mission.md` agent definition. The workflow accepts a successful run based on a valid structured result containing the complete plan and exactly one matching architecture declaration, plus the exact validated architecture artifact at the declared run-context path; both are required. Repository exploration with `codebase-explorer` and the `visualize-change` skill are recommended techniques for producing them, not Factory acceptance gates. The workflow then persists exactly one draft and returns the `pln_` ID. It does not approve, materialize, revise, archive, create missions, run tests, or commit:
...
`status` is `success` or `failure`; artifact paths are repository-relative. For a successful planner run, the structured result must include the complete valid plan and exactly one architecture artifact declaration whose path is the concrete run-context path; Factory validates the declared artifact's existing bytes before persisting the draft. Exploration and skill-use events are diagnostic observability, not acceptance requirements. The result is persisted only after schema validation. Every nonempty backend stdout/stderr stream record is written as a JSON object to the raw JSONL stream (the record includes the stream and raw content; it is not a verbatim line file). Recognized useful events are normalized into SQLite trace events; unknown JSON remains raw-only.
